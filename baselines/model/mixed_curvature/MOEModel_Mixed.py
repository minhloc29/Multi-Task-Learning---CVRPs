...
__all__ = ['MOEModel_Mixed']


class MOEModel_Mixed(nn.Module):
    """
        MOE implementations:
    ...
    def __init__(self, **model_params):
        ...
        self.device = (
            model_params.get('device', torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        )
...
class MTL_Encoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_demand_tw):
        ...
        if isinstance(self.embedding_depot, MoE) or isinstance(self.embedding_node, MoE):
           ...
           embedded_depot_mixed = self.manifold.expmap0(embedded_depot, self.embedding_depot_mixed.c)
           embedded_depot_mixed = self.embedding_depot_mixed(embedded_depot_mixed)
            
           embedded_node_mxied = self.manifold.expmap0(embedded_node, self.embedding_node_mixed.c)
           embedded_node_mxied = self.embedding_node_mixed(embedded_node_mxied)

           embedded_depot = self.manifold.logmap0(embedded_depot_mixed, self.embedding_depot_mixed.c) 
                        
           embedded_node = self.manifold.logmap0(embedded_node_mxied, self.embedding_node_mixed.c)
        ...
        for layer_index,layer in enumerate(self.layers):
            mixed_out = self.manifold.expmap0(out, self.mixed_layers[layer_index].c)
            mixed_out = self.mixed_layers[layer_index](mixed_out)
            out_ = self.manifold.logmap0(mixed_out, self.mixed_layers[layer_index].c)
